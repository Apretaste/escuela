...
class Escuela extends Service
{
	...
	public function _main(Request $request)
	{
		...
		$response->createFromTemplate('basic.tpl', [
			'courses' => array_merge($current_courses, $new_courses, $old_courses)
		]);
		...
		if($course_id > 0)
		{
			...
			if($course->terminated)
			{
				if($course->calification >= 80)
				{
					...
					$html = '<html><body><table width="600" align="center"><tr><td style="border: 1px solid black;">' . '<table width="600" align="center"><tr><td style="border: 5px solid black;">' . '<table width="600" align="center"><tr><td style="border: 1px solid black;">' . '<table width="560" align="center" cellpadding="5" style="margin: 20px;">' . '<tr><td colspan="2" style="font-family: Arial Black; padding: 5px; border: none; font-size: 34px;" align="center">' . '<img height="150" src="' . $this->pathToService . '/sello.jpg"><br/>' . '<strong>CERTIFICACI&Oacute;N<br/> DE CURSO TERMINADO</strong>' . '</td></tr><tr><td colspan="2" style="font-family: Arial; padding: 5px; border: none;" align="center">' . '<br/>Esto certifica que<br/></td></tr><tr>' . '<td colspan="2" style="font-size: 23px; padding: 5px; border: none;" align="center">' . '{$person}' . '<hr/></td></tr><tr>' . '<td colspan="2" style="font-family: Arial; padding: 5px; border: none;" align="center">' . '<br/>ha terminado con &eacute;xito el curso</td></tr><tr>' . '<td colspan="2" align="center" style="font-size: 23px; font-family: Arial; padding: 5px; border: none;">' . '<br/>{$course}<hr/></td></tr><tr>' . '<td width="50%" style="font-size: 20px; padding: 5px; border: none; border: none;" align="center">' . '<br/></td><td style="font-size: 20px; padding: 5px; border: none; border: none;" align="center">' . '<br/>{$teacher_title} {$teacher_name}<hr/></td></tr><tr>' . '<td style="padding: 5px; border: none;" align="center"></td>' . '<td style="font-family: Arial; padding: 5px; border: none;" align="center">Profesor</td>' . '</tr><tr><td colspan="2" align="center" style="padding: 5px; border: none; text-decoration:none;"><i>{$date}</i></td>' . '</tr></table></td></tr></table></td></tr></table></td></tr></table></body></html>';
					...
					$filePath = $www_root . "/temp/$fileName";
	...
	public function _opinar(Request $request)
	{
		...
		if($course !== false)
		{
			...
			if(isset($feedback[0]))
			{
				...
				$feedback_where = " email = '{$request->email}' AND feedback = $feedback_id AND course = $course_id;";
				...
				if($last_answer !== false)
				{
					...
					if($popularity !== false) Connection::query("UPDATE _escuela_course SET popularity = popularity - $popularity WHERE id = $course_id;");
				}
				...
				if($popularity !== false)
				{
					Connection::query("INSERT INTO _escuela_feedback_received (feedback, course, email, answer) VALUES ($feedback_id, $course_id, '{$request->email}', '$answer');");
					Connection::query("UPDATE _escuela_course SET popularity = popularity + $popularity WHERE id = $course_id;");
				}
			}
		}
	...
	public function _repetir(Request $request)
	{
		...
		Connection::query("DELETE FROM _escuela_answer_choosen WHERE course = $course_id AND email = '{$request->email}'");
	...
	private function getChapterImages($chapter_id)
	{
		...
		$imgs = Connection::query("SELECT * FROM _escuela_images WHERE chapter = '$chapter_id';");

		if($imgs === false) $imgs = [];

		$images = [];

		foreach($imgs as $img) $images[ $img->filename ] = $www_root . "/public/courses/{$img->course}/{$img->chapter}/{$img->filename}";
